...
class Client(BaseClient, Database, Page):
    ...
    def clean_pages_data(self, data: dict):
        ...
        for symbol in data:
            ...
            clean_data[symbol] = {
                'website': {'rich_text': [{"text": {
                                                "content": '\n'.join(coin['urls']['website'])
                                            }}]},
                
                'technical_doc':  {'rich_text': [{"text": {
                                                "content": '\n'.join(coin['urls']['technical_doc'])
                                            }}]},
                
                'twitter': {'rich_text': [{"text": {
                                                "content": '\n'.join(coin['urls']['twitter'])
                                            }}]},
                
                'reddit':  {'rich_text': [{"text": {
                                                "content": '\n'.join(coin['urls']['reddit'])
                                            }}]},
                
                'message_board':  {'rich_text': [{"text": {
                                                "content": '\n'.join(coin['urls']['message_board'])
                                            }}]},
                
                'announcement':  {'rich_text': [{"text": {
                ...
                'explorer':  {'rich_text': [{"text": {
                                                "content": '\n'.join(coin['urls']['explorer'])
                                            }}]},
                
                'source_code': {'rich_text': [{"text": {
                ...
                'tags': {'multi_select': [{"name": i } for i in coin['tags']]},
                
                'platform': {'select': {"name": str(coin['platform']) }},
                'category': {'select': {"name": coin['category'] }},
            }
